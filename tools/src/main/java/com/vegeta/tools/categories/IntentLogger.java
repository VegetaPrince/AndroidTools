...
//private IntentLogger() {}
    ...
    public static void dump(Intent intent) {
        if (intent == null) {
            AndroidTools.log.v("no intent found");
            return;
        }
        Bundle extras = intent.getExtras();
        AndroidTools.log.v("Intent[@" + Integer.toHexString(intent.hashCode()) + "] content:");
        AndroidTools.log.v("Action   : " + intent.getAction());
        AndroidTools.log.v("Category : " + intent.getCategories());
        AndroidTools.log.v("Data     : " + intent.getDataString());
        AndroidTools.log.v("Component: " + intent.getComponent().getPackageName() + "/" + intent.getComponent().getClassName());
        dumpFlags(intent.getFlags());
        AndroidTools.log.v("HasExtras: " + (extras != null && !extras.isEmpty()));
        dumpExtras(extras);
    }
    ...
    public static void dumpFlags(int flags) {
        AndroidTools.log.v("Flags    : " + Integer.toBinaryString(flags));
        for (int flag : FLAGS.keySet()) {
            if ((flag & flags) != 0) {
                AndroidTools.log.v("Flag     : " + FLAGS.get(flag));
            }
        }
    }
    ...
    public static void dumpExtras(Bundle extras) {
        ...
        for (String key : extras.keySet()) {
            AndroidTools.log.v("Extra[" + key + "] :" + String.valueOf(extras.get(key)));
        }
    }
}
